  ...
  return {
    ...
    stamp(c, ctx) {
      ...
      for (let pin = 0; pin < PIN_COUNT; pin++) {
        ...
        // Only register pins with an actual net. Adding -1 → pin pollutes the
        // map with every unconnected pin resolving to the last one iterated.
        if (node !== -1) netToPin.set(node, pin);
