...
defineDevice('fuse', (): Device => ({
  ...
  commit(c, ctx) {
    // Only read current while the fuse is intact. Once blown the branch
    // resistance jumps to R_OPEN and the voltage can be arbitrary; dividing
    // by R_CLOSED would report a nonsensical multi-gigaamp reading.
    if (ctx.s.blown === 1) {
      ctx.s.current = 0;
      return;
    }
  ...
  output(_, ctx) {
    return { blown: ctx.s.blown === 1, current: ctx.s.current ?? 0 };
  },
}));
